...
class StateEstimator():
    def __init__(self):
        ...
        self.grid = Grid([-1 for i in range(grid_width * grid_height)], grid_width, grid_height, MAP_RESOLUTION)
    ...
    def _laser_callback(self, msg):
        ...
        zeroIndex = int(self.angleToRangeIndex(0, msg.angle_min, msg.angle_increment))
        self.current_dist_front = msg.ranges[zeroIndex]
        
        for i, range in enumerate(msg.ranges):
            angle = msg.angle_min + i * msg.angle_increment
            scanTime = msg.header.stamp + rospy.Duration(i * msg.time_increment)
            ...
            self.transformer.waitForTransform('/odom', scan_frame, scanTime, rospy.Duration(5))
            trans,rot = self.transformer.lookupTransform('/odom',scan_frame, scanTime)
            ...
            if not (
                ...
                math.isinf(p_odom_matrix[1,0]) or math.isnan(p_odom_matrix[1,0])
                ):
                # map the obstacle on the grid
                if range < msg.range_max:
                    self.map_obstacle(self.current_x, self.current_y, p_odom_matrix[0,0], p_odom_matrix[1,0])
                else:
                    self.map_free_space(self.current_x, self.current_y, p_odom_matrix[0,0], p_odom_matrix[1,0])
    ...
    def map_obstacle(self, c_x, c_y, x, y):
        ...
        if grid_x != None and grid_y != None and c_x != None and c_y != None:
            self.map_free_space(c_grid_x, c_grid_y, grid_x, grid_y) # map the line between cx and cy as free space
    ...
    def map_free_space(self, c_x, c_y, x, y):
        ...
        for step in range(int(num_steps)):
            ...
            if point_x >= 0 and point_x < self.grid.width and point_y >= 0 and point_y < self.grid.height:
                if self.grid.cell_at(point_x, point_y) != 100: # don't get rid of found obstacles # this is bad if obstacles move around, but good if the robot is mapping a static environment.
    ...
    def moveForDuration(self):
        ...
        print("Move at %s m/s and %s rad/s for %s seconds" %(self.linear_vel, degreeFromRad(self.angular_vel), self.move_for))
    ...
    def moveForTickCount(self):
        rate = rospy.Rate(FREQUENCY)
        print("Move at %s m/s and %s rad/s for %s ticks" %(self.linear_vel, degreeFromRad(self.angular_vel), self.tick_count))
    ...
    def straight_line(self, distance):
        ...
        self.tick_count = round(abs(((distance * FREQUENCY)/LINEAR_VELOCITY)), 0)
        ...
        rospy.sleep(.1) # wait a split second for the laser and odom msgs to definitely come in
        laserEst = self.current_dist_front - startDist
        odomEst = math.sqrt((self.current_x-startX) ** 2 + (self.current_y - startY) ** 2)
        print("Rosbot commanded to go %sm, laser readings estimate %sm, odom estimates %sm" %(distance, laserEst, odomEst))
    ...
    def rotate_rel(self, angle, in_radians = False):
        ...
        if(self.tick_count > 0):
            ...
            self.angular_vel = spin_direction * abs(angle*(FREQUENCY)/self.tick_count) 
        ...
        laserEst = numpy.arccos(startDist/self.current_dist_front) * 180/math.pi 
        odomEst = (self.current_theta - startTheta) * 180/math.pi
        print("Rosbot commanded to turn %s, laser readings estimate %s, odom estimates %s" %(angle*180/math.pi, laserEst, odomEst))
